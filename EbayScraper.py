...
def __GetHTML(query, country, condition='', type='all', alreadySold=True):
    ...
    url = f'https://www.ebay{countryDict[country]}/sch/164/i.html?_from=R40&_nkw=' + parsedQuery + alreadySoldString + conditionDict[condition] + typeDict[type]
...
def __StDev(numberList):
    #print ("StDev" + str(numberList))
    lenge = len(list(numberList))
    #print ("lenge " + str(lenge))
    if len(list(numberList)) == 0: return 0
